...
encode_path_value(JsonValueEncoder1 = #argo_json_value_encoder{}, PathValue = #argo_path_value{}) ->
    JsonArray = lists:foldl(
        fun
            ({field_name, FieldName}, ArrayAcc) when is_binary(FieldName) ->
                [FieldName | ArrayAcc];
            ({list_index, ListIndex}, ArrayAcc) when is_integer(ListIndex) ->
                [ListIndex | ArrayAcc]
        end,
        [],
        PathValue#argo_path_value.segments
    ),
    JsonValue = lists:reverse(JsonArray),
    {JsonValueEncoder1, JsonValue}.
