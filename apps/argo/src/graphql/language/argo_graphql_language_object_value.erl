...
%%%-----------------------------------------------------------------------------
%%% % @format
-module(argo_graphql_language_object_value).
...
-type t() :: #argo_graphql_language_object_value{}.
...
-spec parse(Context, Location) -> ObjectValue when
    Context :: context(), Location :: erl_anno:location(), ObjectValue :: t().
parse(_Context = #{fields := Fields}, Location) when is_list(Fields) ->
    #argo_graphql_language_object_value{
        location = Location,
        fields = Fields
    }.
...
format(Formatter1, #argo_graphql_language_object_value{fields = []}) ->
    Formatter2 = argo_graphql_formatter:write(Formatter1, "{}", []),
    Formatter2;
format(Formatter1, #argo_graphql_language_object_value{fields = Fields}) ->
    Formatter2 = argo_graphql_formatter:write(Formatter1, "{", []),
    Formatter3 = argo_graphql_formatter:shift_right(Formatter2),
    {_Index, Formatter4} = lists:foldl(
        fun(ObjectField, {Index, Formatter3_Acc1}) ->
            ...
            Formatter3_Acc4 = argo_graphql_language_object_field:format(Formatter3_Acc3, ObjectField),
            Formatter3_Acc5 =
