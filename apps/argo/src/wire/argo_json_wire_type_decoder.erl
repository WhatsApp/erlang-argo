...
decode_wire_type(JsonWireTypeDecoder1 = #argo_json_wire_type_decoder{}, JsonValue) ->
    case argo_json:is_object(JsonValue) of
        false ->
            error_with_info(badarg, [JsonWireTypeDecoder1, JsonValue], #{2 => expected_object});
        true ->
            ...
            case argo_json:object_get(<<"type">>, JsonObject) of
                ...
                <<"BYTES">> ->
                    ...
                    {JsonWireTypeDecoder1, WireType};
                <<"FIXED">> ->
                    case argo_json:object_get(<<"length">>, JsonObject) of
                        Length when is_integer(Length) andalso Length >= 0 ->
                            ...
                            {JsonWireTypeDecoder1, WireType};
                        _ ->
                            error_with_info(badarg, [JsonWireTypeDecoder1, JsonValue], #{2 => expected_integer})
                    end;
                <<"BLOCK">> ->
                    JsonOf = argo_json:object_get(<<"of">>, JsonObject),
                    {JsonWireTypeDecoder2, Of} = decode_scalar_wire_type(JsonWireTypeDecoder1, JsonOf),
                    case argo_json:object_get(<<"key">>, JsonObject) of
                        Key when is_binary(Key) ->
                            case argo_json:object_get(<<"dedupe">>, JsonObject) of
                                Dedupe when is_boolean(Dedupe) ->
                                ...
                                _ ->
                                    error_with_info(badarg, [JsonWireTypeDecoder1, JsonValue], #{2 => expected_boolean})
                            end;
                        _ ->
                            error_with_info(badarg, [JsonWireTypeDecoder1, JsonValue], #{2 => expected_string})
                    end;
                <<"NULLABLE">> ->
                    JsonOf = argo_json:object_get(<<"of">>, JsonObject),
                ...
                <<"ARRAY">> ->
                    ...
                    {JsonWireTypeDecoder2, WireType};
                <<"RECORD">> ->
                    case argo_json:object_get(<<"fields">>, JsonObject) of
                        JsonFields when is_list(JsonFields) ->
                            ...
                            {JsonWireTypeDecoder2, WireType};
                        _ ->
                            error_with_info(badarg, [JsonWireTypeDecoder1, JsonValue], #{2 => expected_array})
                    end;
                <<"DESC">> ->
                    ok = check_for_unknown_keys(JsonObject, #{<<"type">> => []}),
...
decode_wire_type_store(JsonWireTypeDecoder1 = #argo_json_wire_type_decoder{}, JsonValue) ->
    case argo_json:is_object(JsonValue) of
        ...
        true ->
            ...
            case argo_json:object_get(<<"type">>, JsonObject) of
                <<"STORE">> ->
                    case argo_json:object_get(<<"types">>, JsonObject) of
                        JsonTypes when is_list(JsonTypes) ->
...
format_error_description(_Key, {invalid_type, JsonValue}) ->
    io_lib:format("invalid type: ~0tP", [JsonValue, 5]);
...
decode_field_wire_type(JsonWireTypeDecoder1 = #argo_json_wire_type_decoder{}, JsonValue) ->
    case argo_json:is_object(JsonValue) of
        ...
        true ->
            JsonObject = JsonValue,
            case argo_json:object_get(<<"name">>, JsonObject) of
                Name when is_binary(Name) ->
                    JsonOf = argo_json:object_get(<<"type">>, JsonObject),
                    {JsonWireTypeDecoder2, Of} = decode_wire_type(JsonWireTypeDecoder1, JsonOf),
                    case argo_json:object_get(<<"omittable">>, JsonObject) of
                        Omittable when is_boolean(Omittable) ->
                            ok = check_for_unknown_keys(JsonObject, #{
                                <<"name">> => [], <<"type">> => [], <<"omittable">> => []
                            }),
...
decode_wire_type_store_entry(JsonWireTypeDecoder1 = #argo_json_wire_type_decoder{}, JsonValue) ->
    case argo_json:is_object(JsonValue) of
        ...
        true ->
            JsonObject = JsonValue,
            case argo_json:object_get(<<"name">>, JsonObject) of
                TypeName when is_binary(TypeName) ->
