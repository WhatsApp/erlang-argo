...
%%%-----------------------------------------------------------------------------
%%% % @format
-module(argo_graphql_language_list_value).
...
-type t() :: #argo_graphql_language_list_value{}.
...
-spec parse(Context, Location) -> ListValue when
    Context :: context(), Location :: erl_anno:location(), ListValue :: t().
parse(_Context = #{list := List}, Location) when is_list(List) ->
    #argo_graphql_language_list_value{
        location = Location,
        list = List
    }.
...
format(Formatter1, #argo_graphql_language_list_value{list = []}) ->
    Formatter2 = argo_graphql_formatter:write(Formatter1, "[]", []),
    Formatter2;
format(Formatter1, #argo_graphql_language_list_value{list = List}) ->
    Formatter2 = argo_graphql_formatter:write(Formatter1, "[", []),
    ...
    {_Index, Formatter4} = lists:foldl(
        fun(Value, {Index, Formatter3_Acc1}) ->
            ...
            Formatter3_Acc4 = argo_graphql_language_value:format(Formatter3_Acc3, Value),
            Formatter3_Acc5 =
