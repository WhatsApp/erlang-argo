...
%%%-----------------------------------------------------------------------------
%%% % @format
-module(argo_graphql_language_object_field_const).
...
-type t() :: #argo_graphql_language_object_field_const{}.
...
-spec parse(Context, Location) -> ObjectFieldConst when
    Context :: context(), Location :: erl_anno:location(), ObjectFieldConst :: t().
parse(_Context = #{name := Name, value := ValueConst = #argo_graphql_language_value_const{}}, Location) when
    is_binary(Name)
->
    #argo_graphql_language_object_field_const{
        location = Location,
        name = Name,
        value = ValueConst
    }.
...
format(Formatter1, #argo_graphql_language_object_field_const{name = Name, value = ValueConst}) ->
