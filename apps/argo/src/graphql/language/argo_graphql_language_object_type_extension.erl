...
%%%-----------------------------------------------------------------------------
%%% % @format
-module(argo_graphql_language_object_type_extension).
...
-type t() :: #argo_graphql_language_object_type_extension{}.
...
-spec parse(Context, Location) -> ObjectTypeExtension when
    Context :: context(), Location :: erl_anno:location(), ObjectTypeExtension :: t().
...
->
    ...
    #argo_graphql_language_object_type_extension{
        ...
        directives = OptionDirectivesConst,
        fields = OptionFields
    }.
...
is_ambiguous(#argo_graphql_language_object_type_extension{fields = none}) ->
    true;
is_ambiguous(#argo_graphql_language_object_type_extension{fields = {some, _}}) ->
    false.
...
format(Formatter1, #argo_graphql_language_object_type_extension{
    name = Name, implements = OptionImplements, directives = OptionDirectivesConst, fields = OptionFields
}) ->
    Formatter2 = Formatter1,
    Formatter3 = argo_graphql_formatter:write(Formatter2, "extend type ~ts", [Name]),
    Formatter4 =
    ...
    Formatter5 =
        case OptionDirectivesConst of
            {some, DirectivesConst} ->
                argo_graphql_language_directives_const:format(Formatter4, DirectivesConst);
            none ->
                Formatter4
        end,
    Formatter6 =
